...
public class LibraryDaoJdbcTemplateImpl implements LibraryDao {
    ...
    //Prepared Sattements for AUTHORS
    private static final String SQL_INSERT_AUTHOR = "insert into authors (first_name,last_name, street, city, state, zip, phone) values (?,?,?,?,?,?,?)";
    private static final String SQL_DELETE_AUTHOR = "delete from authors where author_id=?";
    private static final String SQL_UPDATE_AUTHOR = "update authors set first_name=?, last_name=?, street=?, city=?, zip=?, phone=? where author_id=?";
    private static final String SQL_SELECT_AUTHOR = "select * from authors where author_id=?";
    private static final String SQL_SELECT_AUTHORS_BY_BOOK_ID = "select au.author_id, au.first_name, au.last_name,au.street, au.city, au.state, au.zip, au.phone from authors au join books_authors ba on au.author_id=ba.author_id where ba.book_id=?";
    private static final String SQL_SELECT_ALL_AUTHORS = "select * from authors";
    
    //Prepared Statements for BOOKS and BOOK AUTHORS
    ...
    private static final String SQL_DELETE_BOOKS_AUTHORS="delete from books_authors where book_id=?";
    private static final String SQL_UPDATE_BOOK="update books set isbn=?, title=?, publisher=?, price=?, publish_date=? where book_id=?";
    ...
    private static final String SQL_SELECT_BOOKS_BY_AUTHOR_ID="select b.book_id, b.isbn, b.title, b.publisher_id, b.price, b.publish_date from books b join books_authors ba where b.book_id=ba.author_id and ba.author_id";
    
    //Prepared Statements for Publishers
    public static final String SQL_SELECT_BOOKS_BY_PUBLISHER_ID="select * from books where publisher_id=?";
    public static final String SQL_INSERT_PUBLISHER="insert into publishers (name, street, city, state, zip, phone) values (?,?,?,?,?,?)";
    public static final String SQL_DELETE_PUBLISHER="delete from publishers where publisher_id=?";
    public static final String SQL_UPDATE_PUBLISHER="update publisher set name=?, street=?, city=?, state=?, zip=?, phone=? where publisher_id=?";
    public static final String SQL_SELECT_PUBLISHER="select * from publishers where publisher_id=?";
    public static final String SQL_SELECT_PUBLISHER_BY_BOOK_ID="select pub.publisher_id, pub.name, pub.street, pub.city, pub.state, pub.zip, pub.phone from publishers pub join  books on pub.publisher_id=books.publisher_id where books.book_id=?";
    public static final String SQL_SELECT_ALL_PUBLISHERS="select * from publishers";
    ...
    public void updateAuthor(Author author) {
        jdbcTemplate.update(SQL_UPDATE_AUTHOR,author.getFirstName(), author.getLastName(),author.getStreet(),author.getCity(),author.getState(),author.getZip(),author.getPhone(),author.getAuthorId());
    }
    ...
    public void addBook(Book book) {
       jdbcTemplate.update(SQL_INSERT_BOOK,book.getIsbn(), book.getTitle(),book.getPublisher().getPublisherId(),book.getPrice(), book.getPublishDate().toString());
    ...
    public void updateBook(Book book) {
        jdbcTemplate.update(SQL_UPDATE_BOOK,book.getIsbn(),book.getTitle(),book.getPublisher().getPublisherId(),book.getPrice(),book.getPublishDate().toString(),book.getBookId());
    ...
    public void addPublisher(Publisher publisher) {
        jdbcTemplate.update(SQL_INSERT_PUBLISHER, publisher.getName(), publisher.getStreet(), publisher.getCity(), publisher.getState(),publisher.getZip(), publisher.getPhone());
    ...
    public void updatePublisher(Publisher publisher) {
        jdbcTemplate.update(SQL_UPDATE_PUBLISHER, publisher.getName(), publisher.getStreet(), publisher.getCity(), publisher.getState(), publisher.getZip(), publisher.getPhone(), publisher.getPublisherId());
    }
