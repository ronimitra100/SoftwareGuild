...
public class SuperheroOrganizationAffiliationServiceLayerImpl implements SuperheroOrganizationAffiliationServiceLayer{
    ...
    public List<Integer> getAffiliationIdsLinkedToGivenHeroAndOrg(int heroId, int orgId) {
        ...
        //List<Integer> listOfRelevantAffiliationIds = new ArrayList<Integer>();
        /*List<SuperheroOrganizationAffiliation> listOfRelevantAffiliations 
                = affiliationDao.getAllSuperheroOrganizationAffiliations().stream().filter((a)-> (heroId==a.getHeroId()) && (orgId==a.getOrgId())).map(SuperheroOrganizationAffiliation::getAffiliationId).collect(Collectors.toList());*/
